...
function be_display_posts_shortcode( $atts ) {
	...
	if ( ! empty( $post_type ) ) {
		$args['post_type'] = custom_event_explode( $post_type );
	}
	...
	// If Post IDs.
	if ( $id ) {
		$posts_in         = array_map( 'intval', custom_event_explode( $id ) );
	...
	$post__not_in = array();
	if ( ! empty( $exclude ) ) {
		$post__not_in = array_map( 'intval', custom_event_explode( $exclude ) );
	}
	...
	// Post Status.
	$post_status = custom_event_explode( $post_status );
	$validated   = array();
	...
	if ( ! empty( $taxonomy ) && ! empty( $tax_term ) ) {
		...
		if ( 'current' === $tax_term ) {
			...
			$tax_term = custom_event_explode( $tax_term );
		}
		...
		while (
			...
			$terms                = custom_event_explode( sanitize_text_field( $original_atts[ 'tax_' . $count . '_term' ] ) );
	...
	if ( false !== $post_parent__in ) {
		$args['post_parent__in'] = array_map( 'intval', custom_event_explode( $atts['post_parent__in'] ) );
	}
	if ( false !== $post_parent__not_in ) {
		$args['post_parent__not_in'] = array_map( 'intval', custom_event_explode( $atts['post_parent__in'] ) );
	}
	...
	while ( $dps_listing->have_posts() ) :
		...
		if ( $include_date ) {
			$date = 'relative' === $date_format ? custom_event_relative_date( get_the_date( 'U' ) ) : get_the_date( $date_format );
		} elseif ( $include_date_modified ) {
			$date = 'relative' === $date_format ? custom_event_relative_date( get_the_modified_time( 'U' ) ) : get_the_modified_date( $date_format );
		}
		if ( ! empty( $date ) ) {
 ...
 */
function custom_event_explode( $string = '' ) {
 ...
 */
function custom_event_relative_date( $date ) {
